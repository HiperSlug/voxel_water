...
impl Chunk {
    ...
    pub fn liquid_tick(&mut self, tick: u64) {
        let [front, back] = self.db_masks.swap_mut();
        *back = front.clone(); // TODO: cloneless
        ...
        for z in 1..LEN_U32 - 1 {
            'row: for y in 1..LEN_U32 - 1 {
                ...
                let mut liquid = front.liquid_mask[i] & !PAD_MASK;
                ...
                // down
                {
                    let try_move = liquid & !front.some_mask[ny_i];

                    try_move_row::<0, -1, 0>(voxels, back, dst_to_src, try_move, i, yz_i_3d, tick);
                ...
                for j in 0..4 {
                    for k in 0..4 {
                        ...
                        match DIRS[k] {
                            PosX => {
                                let try_move = group & inv_shift::<1>(!front.some_mask[ny_i]);
                                liquid &= !try_move;
                                try_move_row::<1, -1, 0>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                            }
                            NegX => {
                                let try_move = group & inv_shift::<-1>(!front.some_mask[ny_i]);
                            ...
                            PosZ => {
                                let try_move = group & !front.some_mask[ny_pz_i];
                                liquid &= !try_move;
                                try_move_row::<0, -1, 1>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                            ...
                            NegZ => {
                                let try_move = group & !front.some_mask[ny_nz_i];
                                liquid &= !try_move;
                                try_move_row::<0, -1, -1>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                ...
                for j in 0..4 {
                    for k in 0..4 {
                        ...
                        match DIRS[k] {
                            PosX => {
                                let try_move = group & inv_shift::<1>(!front.some_mask[ny_nz_i]);
                                liquid &= !try_move;
                                try_move_row::<1, -1, -1>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                            ...
                            NegX => {
                                let try_move = group & inv_shift::<-1>(!front.some_mask[ny_pz_i]);
                                liquid &= !try_move;
                                try_move_row::<-1, -1, 1>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                ...
                for j in 0..4 {
                    for k in 0..4 {
                        ...
                        match DIRS[k] {
                            PosX => {
                                let try_move = group
                                    & inv_shift::<1>(!front.some_mask[i])
                                    & shift::<1>(front.some_mask[i]);
                                liquid &= !try_move;
                                try_move_row::<1, 0, 0>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                            ...
                            PosZ => {
                                let try_move =
                                    group & !front.some_mask[pz_i] & front.some_mask[nz_i];
                                liquid &= !try_move;
                                try_move_row::<0, 0, 1>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
                            ...
                            NegZ => {
                                let try_move =
                                    group & !front.some_mask[nz_i] & front.some_mask[pz_i];
                                liquid &= !try_move;
                                try_move_row::<0, 0, -1>(
                                    voxels, back, dst_to_src, try_move, i, yz_i_3d, tick,
                                );
...
fn try_move_row<const X: isize, const Y: isize, const Z: isize>(
    ...
    let dst_some = inv_shift::<X>(masks.some_mask[dst_i_2d]);
    ...
    if success != 0 {
        ...
        masks.some_mask[dst_i_2d] |= add_mask;
        masks.liquid_mask[dst_i_2d] |= add_mask;

        masks.some_mask[src_i_2d] &= keep_mask;
        masks.liquid_mask[src_i_2d] &= keep_mask;
    }
    ...
    while failure != 0 {
        ...
        if priority >= other_priority {
            masks.set(*other_src_i_3d, Some(Voxel::Liquid));
            masks.set(src_i_3d, None);
